...
class Database(mongoDbDatabase: DefaultDB)(implicit ec: ExecutionContext) {
  lazy val users = new UserTable(this, mongoDbDatabase("users"))
}

object Database {
  lazy val mongoDbDriver = new MongoDriver

  def open(servers: List[(String, Int)], databaseName: String)(implicit ec: ExecutionContext): Database = {
